...
def generate_content(user, type, topic, platform, purpose, keywords, length):
    ...
    system_message, user_message = generate_messages(type, topic, platform, length, content_length, purpose)

    content_data = create_content_data(user, type, topic, platform, purpose, keywords, length, system_message, user_message)
    assistant_response = get_assistant_response(user, system_message, user_message, content_data)

    if assistant_response:
        handle_chat_instruction(content_data, platform)
...
def generate_messages(type, topic, platform, length, content_length, purpose):
    if type == ContentTypes.SOCIAL_MEDIA_POST and platform == "LINKEDIN":
        ...
        user_message = ContentPrompt.linkedin_user_message(topic=topic, content_length=content_length)
        ...
        named_entity_for_tweet = TweetUtils.extract_entities(preprocessed_topic)
        ...
        system_message = ContentPrompt.social_media_post_system_message(platform=platform, topic=topic, content_length=content_length)
        user_message = ContentPrompt.social_media_post_user_message(platform=platform, topic=topic, content_length=content_length)
...
def handle_chat_instruction(content_data, platform):
    if content_data.type == ContentTypes.SOCIAL_MEDIA_POST:
        system_chat_prompt = ChatPrompt.social_media_system_chat_prompt(platform=platform)
    else:
...
def content_chat_history(user, content_id):
    ...
    for chat in chats_data:
        history.append({
            ...
            "username": "Content AI" if chat.type == constants.ChatTypes.AI else user.name,
